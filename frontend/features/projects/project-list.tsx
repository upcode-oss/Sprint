...
export function ProjectList() {
  ...
  return <>
    ...
    <Dialog open={open} onOpenChange={setOpen} title="Create project" description="A standard Kanban board is created automatically."><form className="form-grid" onSubmit={submit}><div className="form-grid two"><Field label="Name"><Input required autoFocus value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} /></Field><Field label="Key"><Input required minLength={2} maxLength={20} value={form.key} onChange={(event) => setForm({ ...form, key: event.target.value.toUpperCase().replace(/[^A-Z0-9]/g, "") })} /></Field></div><Field label="Description"><Textarea value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} /></Field><Field label="Status"><Select value={form.status} onChange={(event) => setForm({ ...form, status: event.target.value })}><option value="planned">Planned</option><option value="active">Active</option><option value="on_hold">On hold</option></Select></Field><Field label="Teams" interactive><SearchableMultiSelect options={teams.data?.items.map((team) => ({ value: team.id, label: team.name, description: team.description ?? undefined })) ?? []} values={form.team_ids} onValuesChange={(team_ids) => setForm({ ...form, team_ids })} placeholder="Select teams" searchPlaceholder="Search teams…" /></Field><div className="dialog-actions"><Button type="button" variant="outline" onClick={() => setOpen(false)}>Cancel</Button><Button loading={saving}>Create project</Button></div></form></Dialog>
  </>;
}
