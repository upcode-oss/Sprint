...
export function TeamList() {
  ...
  return <>
    ...
    <Dialog open={open} onOpenChange={setOpen} title={editing ? "Manage team" : "Create team"}><form className="form-grid" onSubmit={submit}><Field label="Name"><Input required autoFocus value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} /></Field><Field label="Description"><Textarea value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} /></Field>{hasPermission("teams.manage_members") ? <Field label="Members" interactive><SearchableMultiSelect options={users.data?.items.map((user) => ({ value: user.id, label: user.display_name, description: `@${user.username} · ${user.email}` })) ?? []} values={form.member_ids} onValuesChange={(member_ids) => setForm({ ...form, member_ids })} placeholder="Select members" searchPlaceholder="Search users…" /></Field> : null}<div className="dialog-actions"><Button type="button" variant="outline" onClick={() => setOpen(false)}>Cancel</Button><Button loading={saving}>{editing ? "Save changes" : "Create team"}</Button></div></form></Dialog>
  </>;
}
