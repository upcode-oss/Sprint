...
export function TeamList() {
  ...
  return <>
    ...
    {loading ? <LoadingState /> : error ? <ErrorState message={error} retry={reload} /> : data?.items.length ? <div className="content-grid">{data.items.map((team) => <Card key={team.id}><div className="split"><span className="metric-icon" style={{ position: "static" }}><Users /></span><Badge>{team.members.length} members</Badge></div><h2 style={{ marginTop: "1rem" }}>{team.name}</h2><p className="muted">{team.description || "No description provided."}</p><div className="inline-list">{team.members.slice(0, 8).map((member) => <Badge key={member.id}>{member.first_name} {member.last_name}</Badge>)}{team.members.length > 8 ? <Badge>+{team.members.length - 8}</Badge> : null}</div>{hasPermission("teams.edit") ? <div className="form-actions"><Button variant="outline" size="sm" onClick={() => editDialog(team)}>Manage team</Button></div> : null}</Card>)}</div> : <EmptyState title="No teams yet" description="Create a team to grant project access." />}
    <Dialog open={open} onOpenChange={setOpen} title={editing ? "Manage team" : "Create team"}><form className="form-grid" onSubmit={submit}><Field label="Name"><Input required autoFocus value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} /></Field><Field label="Description"><Textarea value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} /></Field>{hasPermission("teams.manage_members") ? <Field label="Members" hint="Hold Ctrl/Cmd to select several users."><Select multiple value={form.member_ids} onChange={(event) => setForm({ ...form, member_ids: Array.from(event.target.selectedOptions, (option) => option.value) })}>{users.data?.items.map((user) => <option key={user.id} value={user.id}>{user.first_name} {user.last_name} (@{user.username})</option>)}</Select></Field> : null}<div className="dialog-actions"><Button type="button" variant="outline" onClick={() => setOpen(false)}>Cancel</Button><Button loading={saving}>{editing ? "Save changes" : "Create team"}</Button></div></form></Dialog>
  </>;
}
