...
export function ProfileManager() {
  ...
  return <>
    ...
    <div className="content-grid profile-grid">
      ...
      <Card><CardHeader title="Preferences" description="Personal timezone and locale preferences." /><form className="form-grid" onSubmit={saveProfile}><Field label="Timezone" hint="Any valid IANA timezone, for example Europe/Vienna."><Input required list="timezone-options" value={profile.timezone} onChange={(event) => setProfile({ ...profile, timezone: event.target.value })} /><datalist id="timezone-options">{commonTimezones.map((timezone) => <option key={timezone} value={timezone} />)}</datalist></Field><Field label="Locale" hint="Optional profile locale, for example en-US. The interface is always English."><Input placeholder="System default" value={profile.locale} onChange={(event) => setProfile({ ...profile, locale: event.target.value })} /></Field><div className="form-actions"><Button loading={saving}><Save />Save preferences</Button></div></form></Card>
