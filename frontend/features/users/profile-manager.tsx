...
const commonTimezones = ["UTC", "Europe/Vienna", "Europe/Berlin", "Europe/London", "America/New_York", "America/Los_Angeles", "Asia/Tokyo"];
...
export function ProfileManager() {
  ...
  return <>
    ...
    <div className="content-grid profile-grid">
      ...
      <Card><CardHeader title="Preferences" description="Language and local date/time context." /><form className="form-grid" onSubmit={saveProfile}><Field label="Timezone" hint="Any valid IANA timezone, for example Europe/Vienna."><Input required list="timezone-options" value={profile.timezone} onChange={(event) => setProfile({ ...profile, timezone: event.target.value })} /><datalist id="timezone-options">{commonTimezones.map((timezone) => <option key={timezone} value={timezone} />)}</datalist></Field><Field label="Language" hint="Optional locale such as de-AT or en-US."><Input placeholder="System default" value={profile.locale} onChange={(event) => setProfile({ ...profile, locale: event.target.value })} /></Field><div className="form-actions"><Button loading={saving}><Save />Save preferences</Button></div></form></Card>
