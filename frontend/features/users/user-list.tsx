...
export function UserList() {
  ...
  async function submit(event: FormEvent) { event.preventDefault(); setSaving(true); try { if (editing) { const { password: _password, ...payload } = form; await api(`/users/${editing.id}`, { method: "PATCH", body: jsonBody(payload) }); } else { await api("/users", { method: "POST", body: jsonBody(form) }); } setOpen(false); setForm(blank); notify(editing ? "User updated" : "User created"); await reload(); } catch (reason) { notify((reason as Error).message, "error"); } finally { setSaving(false); } }
