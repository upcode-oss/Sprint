...
export default function ForgotPasswordPage() {
  ...
  return <main className="centered-page"><Card className="auth-card"><div className="auth-heading"><OrganizationBrandMark className="auth-brand-mark" /><h1>Reset password</h1><p>We will send a reset link when SMTP is available.</p></div><form className="form-grid" onSubmit={submit}><Field label="Email"><Input required type="email" value={email} onChange={(event) => setEmail(event.target.value)} /></Field>{message ? <p className="muted" role="status">{message}</p> : null}<Button loading={loading}>Send reset link</Button></form><div className="auth-links"><Link href="/login">Back to sign in</Link></div></Card></main>;
}
